...
def process(departements, forceload, **kwargs):
    ...
    for dept in sorted(departements):
        ...
        if status or forceload:
            import_to_pg(dept)
    sql_process('table_cadastre_communes_lineaire',{})


def download(departement):
    id_batch = b.batch_start_log("download source", "COMMUNES CADASTRE", departement)
    ...
    resp = requests.get(
        f"https://cadastre.data.gouv.fr/data/etalab-cadastre/latest/geojson/departements/{departement}/cadastre-{departement}-communes.json.gz",
        headers=headers,
    )
...
def import_to_pg(departement, **kwargs):
    id_batch = b.batch_start_log("import source", "COMMUNES CADASTRE", departement)
    ...
    with gzip.open(fichier_source, mode="rt") as f:
        ...
        with db.bano_db.cursor() as cur_insert:
            try:
                cur_insert.execute(
                    f"DELETE FROM cadastre_communes WHERE code_insee LIKE '{departement+'%'}';COMMIT;"
                )
                a_values = []
                str_query = f"INSERT INTO cadastre_communes VALUES "
                for l in json_source["features"]:
                    nom = hp.escape_quotes(l['properties']['nom']).replace("\\","")
                    created = f"'{l['properties']['created']}'" if 'created' in l['properties'] else 'null'
                    updated = f"'{l['properties']['updated']}'" if 'updated' in l['properties'] else 'null'
                    a_values.append(
                        f"('{l['properties']['id']}','{nom}',{created},{updated},ST_SetSRID(ST_Multi(ST_GeomFromGeoJSON('{hp.replace_single_quotes_with_double(str(l['geometry']))}')),4326))"
                    )
                if a_values:
...
def get_destination(departement):
    ...
    return cwd / f"cadastre-{departement}-communes.json.gz"

...
def download(departement):
    ...
    resp = requests.get(
        f"https://adresse.data.gouv.fr/data/ban/adresses-odbl/latest/csv/adresses-{departement}.csv.gz",
        headers=headers,
    )
    ...
    if resp.status_code == 200:
        ...
        if destination.exists() and mtime <= destination.stat().st_mtime:
...
def import_to_pg_subp(departement, **kwargs):
    ...
    try:
        fichier_source = get_destination(departement)
        ret = subprocess.run(
            ["gzip", "-cd", fichier_source], capture_output=True, text=True
        )
